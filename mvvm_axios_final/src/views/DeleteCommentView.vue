    ...
    <div>
        <div class="alert alert-dismissible alert-danger">
            ...
            <button type="button" class="btn btn-outline-danger" style="margin-right: 10px"
                v-on:click="this.delete()">Oui</button>
            <button type="button" class="btn btn-outline-info" style="margin-left: 10px"
                v-on:click="this.redirect()">Non</button>
        </div>
    </div>
</template>
...
export default
    {
        ...
        {
            delete() {
                let request = 'http://localhost:3000/wp-json/wp/v2/comments/' + this.$route.params.id + '?force=true';
